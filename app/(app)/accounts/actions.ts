...
async function resolveArtFor(
  ...
  /* TEMPORARY: no gate but the account type. Every card save re-infers, so the
     whole table can be regenerated by re-saving — the card-art prompt changed
     under accents written before it, and the stored greys are what this exists
     to replace. The gate goes back in the commit after this one; do not leave
     this deployed. */
  void previous;
