...
describe("resolveSubscriptionBrand runs once per name, not once per save", () => {
  ...
  // TEMPORARILY SKIPPED: the gate is out for one commit so the table can be
  // regenerated by re-saving. Restored with the gate in the next commit.
  it.skip("does NOT infer again once an attempt is recorded", async () => {
    const writes = stub({
  ...
  // TEMPORARILY SKIPPED: the gate is out for one commit so the table can be
  // regenerated by re-saving. Restored with the gate in the next commit.
  it.skip("does NOT infer when the row already has a logo", async () => {
    const writes = stub({
