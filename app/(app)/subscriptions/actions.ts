...
export async function resolveSubscriptionBrand(id: string): Promise<{ resolved: boolean }> {
  ...
  /* TEMPORARY: no gate at all. Every save re-infers, so the whole table can be
     regenerated by re-saving rows — the brand prompt and the icon lookup both
     changed under data that was written before them. The gate goes back in the
     commit after this one; do not leave this deployed. */
