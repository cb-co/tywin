...
| `saveMerchantRule` (`accounts/statement-actions.ts`) | direct | Single `upsert`. Note: unlike `updateRule` below, this action does not re-check that `categoryId` belongs to the caller before writing it — see the report's close-call note; not something this task changes. | direct Supabase write |
