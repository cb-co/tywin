-- Cashback earned, captured per statement.
--
-- The figure is a SIBLING OF THE BALANCE ANCHOR: `card_statements.total_balance`
-- is what a card's balance is anchored to, and `cashback_total` sits beside it
-- on the same row, carrying what the issuer credited for that same closing
-- period. Storing it per statement rather than as a running per-account total
-- is what makes re-uploading a statement safe: the import RPC already deletes
-- and re-inserts the row for a given (account_id, period_end) — see the delete
-- below the checksum guard — so a second upload of the same PDF overwrites one
-- row instead of adding one. The year figure is then a SUM over the statements
-- whose period_end falls in that year: every new statement sums onto the
-- running total, and deleting a statement removes its contribution, with no
-- accumulator that can drift out of step with the statements behind it.
--
-- Nullable with no default: NULL means "this statement never reported one"
-- (every row imported before this migration), which is deliberately different
-- from 0.00, "the statement reported no cashback this period".
alter table public.card_statements
  add column cashback_total numeric(18,4);

-- Statement import: capture cashback_total. Body is otherwise identical to
-- 20260731130000_card_payment_default_and_cashflow.sql's definition — the true
-- latest `import_card_statement` before this migration — preserving the
-- checksum guard (20260722130000), the duplicate-section rejection
-- (20260722150000), the fx_fallback capture (20260722160000), and the
-- exclude_from_budget default (20260731130000).
