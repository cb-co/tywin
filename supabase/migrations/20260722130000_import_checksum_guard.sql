-- Checksum enforcement at write boundary
-- Review finding on 20260722120000: app-layer validation can be bypassed by
-- calling the RPC directly. The statement's own arithmetic (previous_balance
-- + movement = total_balance) is now enforced inside the database boundary,
-- preventing invalid imports from reaching tables.
