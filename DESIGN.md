...
| **Stamp** (double ink ring + glyph/emoji/initial in the stored hex) | `ColorTile` | categories, accounts, merchants, the Accounts attention ledger's lead, and now Phase 3's ledger rows and the `CategoryRail` stamp picker | built (`stamp.tsx`, `lib/papel/ink.ts`); `ColorTile` down to 7 callers |
